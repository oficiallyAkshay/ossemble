...
Output shape: `references/contract.md` section 7, and nothing else. If you cannot find a citable fact for something, write the words `not verified` for that item instead of guessing or inferring. Never state an opinion, a recommendation, or a verdict; that is the model's job at a later step, not yours.
