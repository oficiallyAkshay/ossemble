...
Output shape: `references/contract.md` section 7. Mark a finding `confirmed` only when you personally read the exact lines and can point to the break; otherwise mark it `plausible` and say what would confirm it. A finding with no rule id is still valid; not everything wrong has a rule yet.
